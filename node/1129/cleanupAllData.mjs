...
function cleanupData(fileName) {
    ...
    fs.createReadStream(path.resolve(__dirname, 'node', '1129', 'input', `${fileName}.csv`))
        .pipe(csv.parse())
        ...
        .on('end', (rowCount) => {
            ...
            csv.writeToPath(path.resolve(__dirname, 'node', '1129', 'output', `${fileName}.csv`), rows)
                .on('error', err => console.error(err))
                .on('finish', () => console.log('Done writing.'));
        });
}

['origin_2002', 'origin-c1_2002', 'origin-c2_2002', 'origin-c3_2002'].forEach(c => {
    cleanupData(c);
})
